...
def run(multiagent=DEFAULT_MA, output_folder=DEFAULT_OUTPUT_FOLDER, gui=DEFAULT_GUI, plot=True, colab=DEFAULT_COLAB, record_video=DEFAULT_RECORD_VIDEO, local=True):
    ...
    class CustomMlpExtractor(BaseFeaturesExtractor):
        def __init__(self, observation_space: spaces.Box, features_dim: int = 32):
            ...
            self.shared_net = nn.Sequential(
                ...
                nn.ReLU(),  # Custom activation function
    ...
    eval_callback = EvalCallback(eval_env,
                                 ...
                                 deterministic=True,
                                 render=False)
    model.learn(total_timesteps=int(1500000) if local else int(1e2), # shorter training in GitHub Actions pytest
    ...
    print("Server error nhi")
    print("\n\n\nMean reward ", mean_reward, " +- ", std_reward, "\n\n")                  # Here I guess the policy may or may not be optimized but evaluation of the episodes is surely being done; 
                                                                                        # does this make it offline method?
